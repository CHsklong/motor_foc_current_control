 ...
 * S 曲线轨迹规划（2026-09-15）
 *
 * 原来只有"接近限速"：靠 sqrt(2*a*|err|) 强制电机在靠近目标时减速。
 * 它解决的是"冲过目标"，但起步/中段仍是"速度给定阶跃 + 速度环自己爬"，
 * 加速度在指令上是突变的，机械上有冲击。
 *
 * S 曲线把速度给定本身规划成平滑曲线：
 *   加速度先匀升（jerk J）→ 匀加速 → 匀降 → 匀速 → 对称减速
 * 速度就是 S 形，位置连续可导，起停无冲击。
 *
 * 【为什么必须加速度前馈】
 * 位置环是 P 为主（kp=20），跟踪一个以 v 匀速移动的目标时会有跟随误差
 *   e_ss ≈ v / kp = 25/20 = 1.25 rad
 * 也就是说末端会欠 1.25 rad 才停，S 曲线白做了。所以要把规划速度 ω*(t)
 * 作为前馈直接加到位置环输出上，PID 只补偿模型误差与扰动（见 POS_TRACK_VLIM_MIN）。
 *
 * 【约束怎么选】
 *   V_MAX 25 rad/s ≈ 239 rpm，略低于 output_max 30，给 PID 留修正余量
 *   A_MAX 150 rad/s²：J=6.2e-6 → 转矩 9.3e-4 N·m → 电流约 0.02A，远未饱和
 *   J_MAX 3000 rad/s³：加速度 0.05s 内升到 150，冲击足够柔和
 * 行程 39.27rad 时规划结果：tj=0.05s, ta=0.217s, tv=1.354s, 总时长 1.79s。
 * 想更快就加大 A_MAX / J_MAX（注意别让电流环饱和），或提高 V_MAX。
 * ========================================================================== */

/** 1=启用 S 曲线规划；0=回到原来的固定目标 + 接近限速 */
#define POS_SCURVE_ENABLE  (0)

/** 巡航速度上限 rad/s（应小于 POSITION_OUTPUT_MAX，留出 PID 修正余量） */
#define POS_SCURVE_V_MAX   (25.0f)

/** 最大加速度 rad/s² */
#define POS_SCURVE_A_MAX   (150.0f)

/** 最大急动度 rad/s³（加速度的变化率，决定起步/停止的柔和程度） */
#define POS_SCURVE_J_MAX   (3000.0f)

/** 速度前馈增益，1.0=完全前馈；跟不上可略降到 0.8~0.9 */
#define POS_SCURVE_FF_GAIN (1.0f)

/** 轨迹期 PID 限幅的下限 rad/s：即便前馈把预算吃光，也要给 PID 留这点修正权
 *
 * 轨迹跟踪阶段位置环的限幅 = clamp(OUTPUT_MAX − |前馈|, POS_TRACK_VLIM_MIN, OUTPUT_MAX)，
 * 即"前馈吃剩多少就给 PID 多少，但至少保底这么多"。
 * 【2026-09-15 踩过的坑】这里原先还要再被一个固定上限（8rad/s）截一刀，
 * 起步段前馈≈0、预算本来是满的 30，被砍到 8 之后一旦静摩擦偏大就没有裕度把电机拽动 ——
 * 这正是"SCURVE=1 完全不转、SCURVE=0 却转得好好的"的主因。别再加回固定上限。 */
#define POS_TRACK_VLIM_MIN (4.0f)

/** 失速保护阈值 rad：轨迹在跑但跟随误差超过它，判定为"电机没跟上"，
 *  直接放弃轨迹退回固定目标 + 接近限速（那条路径已验证能转），保证一定能走到终点。
 *  正常跟踪时跟随误差只有 0.0x rad，3.0 留了几十倍余量。 */
#define POS_SCURVE_ABORT_ERR (3.0f)
